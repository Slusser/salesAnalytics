...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const currentUser = request.currentUser as CustomerMutatorContext | undefined

    if (!currentUser) {
      throw new ForbiddenException('Brak uprawnień do wykonania operacji.')
    }

    const hasRole = currentUser.actorRoles.some((role) => requiredRoles.includes(role))

    if (!hasRole) {
      throw new ForbiddenException('Brak wymaganych ról do wykonania operacji.')
    }

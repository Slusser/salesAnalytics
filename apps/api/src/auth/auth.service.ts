...
export class AuthService {
  ...
  async login(command: AuthLoginCommand): Promise<AuthLoginResponse> {
    ...
    if (signInResult.error) {
      this.logger.warn(`Nieudane logowanie użytkownika ${command.email}`, signInResult.error)
    ...
    if (!session) {
      ...
      throw new UnauthorizedException({ code: 'invalid_credentials', message: 'Nieprawidłowe dane logowania.' })
    }

    const roles = await this.fetchUserRoles(session.user.id, session.access_token)
  ...
  private async fetchUserRoles(userId: string, accessToken: string): Promise<UserRoleValue[]> {
    ...
    if (!data || data.length === 0) {
      this.logger.warn(`Użytkownik ${userId} nie posiada przypisanych ról. Zwracam pustą listę.`)
      return []
    }
  ...
  private mapSignInError(message: string): UnauthorizedException {
    ...
    let code: 'invalid_credentials' | 'wrong_passoword' | 'user_not_found' = 'invalid_credentials'
    ...
    return new UnauthorizedException({ code, message: 'Nieprawidłowe dane logowania.' })
  }
}

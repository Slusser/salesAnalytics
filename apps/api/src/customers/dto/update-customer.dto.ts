...
const transformOptionalString = ({ value }: { value: unknown }): string | undefined => {
...
const transformOptionalBoolean = ({ value }: { value: unknown }): boolean | undefined => {
...
const transformOptionalDate = ({ value }: { value: unknown }): string | null | undefined => {
  if (value === undefined || value === null || value === '') {
    return undefined
  }
...
export class UpdateCustomerDto {
  ...
  @IsDateString({}, { message: 'Pole deletedAt musi być poprawną datą w formacie ISO 8601.' })

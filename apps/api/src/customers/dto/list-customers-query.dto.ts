...
const toBoolean = ({ value }: TransformFnParams, defaultValue: boolean): boolean | unknown => {
  if (value === undefined || value === null || value === '') {
    return defaultValue
  }
...
const normalizeSearch = ({ value }: TransformFnParams): string | undefined | unknown => {
...
export class ListCustomersQueryDto implements ListCustomersQuery {
  ...
  @ApiPropertyOptional({
    description: 'Czy do wyniku należy dołączyć nieaktywnych klientów (wymaga roli editor/owner).',
    default: false,
    example: false
  })
  @IsOptional()
  @Transform((params) => toBoolean(params, false))
  @IsBoolean({ message: 'Parametr includeInactive musi być wartością logiczną.' })

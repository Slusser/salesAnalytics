...
const toInt = ({ value }: TransformFnParams, defaultValue: number): number | unknown => {
...
const normalizeOrderNo = ({ value }: TransformFnParams): string | undefined | unknown => {
...
const normalizeSort = ({ value }: TransformFnParams): string | undefined | unknown => {
  if (value === undefined || value === null) {
    return undefined
  }
...
export class ListOrdersQueryDto implements ListOrdersQuery {
  ...
  @Max(MAX_LIMIT, { message: `Parametr limit nie może przekraczać ${MAX_LIMIT}.` })
  limit?: number

  @ApiPropertyOptional({
    description: 'Identyfikator klienta (UUID), dla którego filtrujemy zamówienia.',
  ...
  @IsOptional()
  @IsDateString({}, { message: 'Parametr dateFrom musi być poprawną datą w formacie ISO 8601.' })
  ...
  @IsOptional()
  @IsDateString({}, { message: 'Parametr dateTo musi być poprawną datą w formacie ISO 8601.' })
  dateTo?: string
  ...
  @ApiPropertyOptional({
    description: 'Czy dołączyć rekordy miękko usunięte (wymaga roli editor/owner).',
    example: false,
    default: false
  })
  @IsOptional()
  @Transform(toBoolean)
  @IsBoolean({ message: 'Parametr includeDeleted musi być wartością logiczną.' })

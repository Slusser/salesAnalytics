...
import { catchError, distinctUntilChanged, finalize, map, of, shareReplay, tap } from 'rxjs'

import type { ListOrdersQuery, ListOrdersResponse, OrderListItemDto } from 'apps/shared/dtos/orders.dto'
...
const DEFAULT_SORT: OrdersSortState = Object.freeze({ field: 'orderDate', direction: 'desc' })
...
export class OrdersListService {
  ...
  readonly permissions = signal<OrdersRolePermissionsVm>({ canMutate: false, canIncludeDeleted: false })

  private readonly initialParams = this.normalizeParams(this.route.snapshot.queryParamMap)

  private readonly paramsSignal = toSignal(
    this.route.queryParamMap.pipe(map((params) => this.normalizeParams(params))),
    { initialValue: this.initialParams }
  )

  readonly params = signal<OrdersQueryParamsVm>(this.paramsSignal() ?? this.initialParams)
  readonly loading = signal(false)
  readonly error = signal<unknown>(null)
  readonly data = signal<OrdersListVm | null>(null)

  private readonly confirmation = signal<ConfirmationState>({ open: false, title: '', loading: false })
  ...
  readonly confirmDialogOrderNo = computed(() => this.confirmation().order?.orderNo ?? '')
  ...
  readonly showEmpty = computed(() => !this.loading() && !this.error() && this.items().length === 0)
  ...
  setParams(partial: Partial<OrdersQueryParamsVm>, options: { resetPage?: boolean } = {}): void {
  ...
  toggleIncludeDeleted(enabled: boolean): void {
    if (!this.permissions().canIncludeDeleted) {
      this.message.warning('Nie masz uprawnień do przeglądania usuniętych zamówień.')
      return
    }
  ...
  handlePaginationChange(page: number, limit?: number): void {
    ...
    this.setParams(partial, { resetPage: Boolean(limit && limit !== this.params().limit) })
  }
    ...
    this.requestExport(payload)
      .pipe(
        tap((blob) => {
          ...
          this.triggerDownload(blob, `orders-${this.buildExportSuffix(payload.timestamp)}.xlsx`)
  ...
  private initializePermissions(): void {
    effect(
      () => {
        ...
        const canMutate = roles.includes('owner' as AppRole) || roles.includes('editor' as AppRole)
  ...
  private requestOrders(params: OrdersQueryParamsVm) {
    ...
    return this.http.get<ListOrdersResponse>('/api/orders', { params: httpParams }).pipe(shareReplay(1))
  }
  ...
  private performMutation(order: OrderRowVm, action: 'soft-delete' | 'restore'): void {
    if (this.confirmation().loading) {
      return
    }
  ...
  private handleFetchError(error: unknown, params: OrdersQueryParamsVm): void {
    ...
    if (this.isForbiddenError(error) && params.includeDeleted) {
      this.message.warning('Brak uprawnień do podglądu usuniętych zamówień. Przywrócono filtr domyślny.')
      this.setParams({ includeDeleted: undefined })
      return
    }

    if (this.isValidationError(error)) {
      this.message.error('Niepoprawne parametry wyszukiwania zamówień. Sprawdź filtry.')
      return
    }

    this.notification.error('Nie udało się pobrać zamówień', 'Spróbuj ponownie później.')
  }

  private handleMutationError(error: unknown, action: 'soft-delete' | 'restore'): void {
  ...
  private handleExportError(error: unknown): void {
    ...
    this.message.error('Nie udało się wyeksportować zamówień. Spróbuj ponownie później.')
  }
  ...
  private resetConfirmation(): void {
    this.confirmation.set({ open: false, action: undefined, order: undefined, title: '', loading: false })
  }
  ...
  private normalizeParams(source: Params | any): OrdersQueryParamsVm {
    const getValue = (key: string): unknown => (source?.get ? source.get(key) : source[key])
    ...
    const { dateFrom, dateTo } = this.normalizeDateRangeFromParams(getValue('dateFrom'), getValue('dateTo'))

    return this.normalizeState({ page, limit, sort, orderNo, customerId, dateFrom, dateTo, includeDeleted })
  }

  private normalizeState(partial: Partial<OrdersQueryParamsVm>): OrdersQueryParamsVm {
    ...
    const { dateFrom, dateTo } = this.ensureDateRange(partial.dateFrom, partial.dateTo)
  ...
  private ensureDateRange(dateFrom?: string, dateTo?: string): { dateFrom?: string; dateTo?: string } {
    if (!dateFrom && !dateTo) {
      return {}
    }
  ...
  private ensureIncludeDeleted(value: boolean | undefined): boolean | undefined {
    if (!value) {
      return undefined
    }
  ...
  private normalizeDateRangeFromParams(dateFrom: unknown, dateTo: unknown): {
    dateFrom?: string
    dateTo?: string
  } {
  ...
  private toQueryParams(vm: OrdersQueryParamsVm): Params {
    return {
      ...
      ...(vm.customerId ? { customerId: vm.customerId } : { customerId: undefined }),
      ...(vm.dateFrom ? { dateFrom: vm.dateFrom } : { dateFrom: undefined }),
      ...(vm.dateTo ? { dateTo: vm.dateTo } : { dateTo: undefined }),
      ...(vm.includeDeleted ? { includeDeleted: true } : { includeDeleted: undefined })
    }
  }
  ...
  private mapRow(dto: OrderListItemDto): OrderRowVm {
    ...
    return {
      ...
      grossFormatted: this.formatCurrency(dto.totalGrossPln, dto.isEur ? 'EUR' : 'PLN'),
  ...
  private normalizeSortDirection(direction: string): OrdersSortDirection | null {
  ...
  private areParamsEqual(a: OrdersQueryParamsVm, b: OrdersQueryParamsVm): boolean {
    return (
      a.page === b.page &&
      a.limit === b.limit &&
  ...
  private extractStatus(error: unknown): number | null {
    if (typeof error === 'object' && error) {
      ...
      const nestedStatus = (error as { error?: { status?: number; statusCode?: number } }).error?.status ?? (

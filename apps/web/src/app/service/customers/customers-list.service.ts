...
export class CustomersListService {
  ...
  private readonly paramsSignal = toSignal(this.route.queryParamMap.pipe(map((params) => this.normalizeParams(params))), {
    initialValue: this.normalizeParams(this.route.snapshot.queryParamMap)
  })

  readonly params = signal<CustomersQueryParamsVm>(this.paramsSignal() as CustomersQueryParamsVm)
  ...
  resetFilters(): void {
    this.setParams({ search: undefined, includeInactive: undefined }, { resetPage: true })
  }

  setParams(partial: Partial<CustomersQueryParamsVm>, options: { resetPage?: boolean } = {}): void {
  ...
  private requestCustomers(params: CustomersQueryParamsVm) {
  ...
  private normalizeState(partial: Partial<CustomersQueryParamsVm>): CustomersQueryParamsVm {
  ...
  private toQueryParams(vm: CustomersQueryParamsVm): Params {
    return {
      ...
      ...(vm.includeInactive ? { includeInactive: true } : { includeInactive: undefined })
    }
  }
  ...
  private toHttpParams(dto: ListCustomersQuery): Record<string, string> {
    const params: Record<string, string> = {
      page: String(dto.page ?? CUSTOMERS_DEFAULT_PAGE),
      limit: String(dto.limit ?? CUSTOMERS_DEFAULT_LIMIT)
    }
    if (dto.search) {
  ...
  private ensureLimit(value: number | undefined): number {
    ...
    if (!CUSTOMERS_LIMIT_OPTIONS.includes(value as any)) return CUSTOMERS_DEFAULT_LIMIT
    return value
  }
  ...
  private ensureIncludeInactive(value: boolean | undefined): boolean | undefined {
    if (!this.canMutate()) return undefined
    return value ? true : undefined
  }

  private areParamsEqual(a: CustomersQueryParamsVm, b: CustomersQueryParamsVm): boolean {
    return (
      a.page === b.page &&
      a.limit === b.limit &&
      (a.search ?? '') === (b.search ?? '') &&
  ...
  private mapDtoToVm(customer: CustomerDto): CustomerRowVm {
    return {
      id: customer.id,
      name: customer.name,

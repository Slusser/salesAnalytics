...
import type { AuthLoginResponse, AuthenticatedUserDto, AuthTokensDto } from 'apps/shared/dtos/auth.dto'
...
export class AuthSessionService {
  ...
  readonly user: Signal<AuthenticatedUserDto | null> = computed(() => this.state().user)
  readonly tokens: Signal<AuthTokensDto | null> = computed(() => this.state().tokens)
  readonly isLoggedIn = computed(() => Boolean(this.state().tokens && this.state().user))
  ...
  logout(options?: { redirectTo?: string | null; extras?: NavigationExtras }): void {

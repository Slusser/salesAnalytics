...
import { ORDERS_MAX_ORDER_NO_LENGTH, OrdersFilterFormState } from '../../../../service/orders/orders-list.types'
...
export class OrdersFilterBarComponent implements OnInit, OnDestroy {
  ...
  @Output() readonly filtersChange = new EventEmitter<Partial<OrdersFilterFormState>>()
  ...
  private initializeForm(): void {
    const form = this.fb.group({
      ...
      dateRange: this.fb.control<[Date | null, Date | null] | null>(this.toDateRange(this.value.dateRange))
    })
    ...
    this.subscription.add(
      form.valueChanges
        .pipe(
          ...
          map((value): Partial<OrdersFilterFormState> => ({
  ...
  private toDateRange(range: OrdersFilterFormState['dateRange']): [Date | null, Date | null] | null {
  ...
  private normalizeDateRange(range: [Date | null, Date | null] | null): [string, string] | undefined {
    if (!range || range.length !== 2) {
      return undefined
    }

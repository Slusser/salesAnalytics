...
export class OrdersNewPageComponent {
  ...
  constructor() {
    effect(() => {
      ...
      if (snapshot.formModel.currencyCode === 'EUR' && !snapshot.formModel.eurRate && snapshot.fxRate.status === 'idle') {
  ...
  private handleCreateError(error: any): void {
    ...
    if (!error?.response?.fieldErrors) {
      this.message.error('Nie udało się utworzyć zamówienia. Spróbuj ponownie.')
    }
  }
  ...
  private mapErrorToForm(error: any) {
    ...
    return {
      generalError: response.message ?? 'Nie udało się utworzyć zamówienia. Spróbuj ponownie.'
    }
  }
}

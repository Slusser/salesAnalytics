...
import type { ApiError, CustomerViewModel } from '../../shared/types/customers.view-model'
import { mapToViewModel } from '../../shared/types/customers.view-model'
import type { CustomerFormModel, ServerValidationErrors } from '../../shared/components/customers/customer-form/customer-form.types'
...
export class CustomerDetailPage {
  ...
  protected readonly restoreInProgress = computed(() => this.state().restoreInProgress)
  protected readonly showDeleteDialog = computed(() => this.state().showDeleteDialog)
  protected readonly notFound = computed(() => this.state().notFound)
  protected readonly canEdit = computed(() => this.customer()?.canEdit ?? false)
  protected readonly canRestore = computed(() => this.customer()?.canRestore ?? false)
  protected readonly isSoftDeleted = computed(() => this.customer()?.isSoftDeleted ?? false)
  ...
  protected onRestore(): void {
    ...
    this.service
      ...
      .subscribe({
        ...
        error: (err) => {
          this.handleMutationError(err, 'Nie udało się przywrócić kontrahenta.', 'restore')
        }
      })
  }
  ...
  private loadCustomer(): void {
    ...
    if (!customerId) {
      this.patchState({
        loading: false,
        error: { code: 'CUSTOMERS_DETAIL_INVALID_ID', message: 'Nieprawidłowy identyfikator kontrahenta.' },
        notFound: true
      })
      return
    }
  ...
  private handleMutationError(error: unknown, fallbackMessage: string, context: 'update' | 'restore' | 'delete'): void {
    ...
    if (apiError.code === 'CUSTOMERS_UPDATE_FORBIDDEN' || apiError.code === 'CUSTOMERS_DELETE_FORBIDDEN') {
      this.message.error('Brak uprawnień do wykonania akcji.')
      this.refreshRoles()
      return
    }

    if (apiError.code === 'CUSTOMERS_UPDATE_NOT_FOUND' || apiError.code === 'CUSTOMERS_DELETE_NOT_FOUND') {
  ...
  private mapUpdateValidationError(error: any): ServerValidationErrors | null {
    ...
    if (apiError.code === 'CUSTOMERS_UPDATE_VALIDATION') {
      const fieldErrors: Partial<Record<'name' | 'isActive' | 'comment', string>> = {}
      const details: string[] = Array.isArray(apiError.details) ? apiError.details : []
      ...
      return {
        fieldErrors,
        generalError: details.length ? details.join(' ') : 'Wystąpił błąd walidacji.'
      }
    }
  ...
  private mapApiError(error: any): ApiError {
    ...
    if (status === 500) {
      return {
        ...
        message: apiError?.message ?? 'Wystąpił błąd serwera. Spróbuj ponownie.'
      }
    }
  ...
  private refreshRoles(): void {
    ...
    this.patchState({
      customer: this.customer()
        ? { ...this.customer()!, canEdit: this.hasAnyRole(user.roles), canRestore: this.hasAnyRole(user.roles) && this.customer()!.isSoftDeleted }
        : null
    })
  }
  ...
  private setupFocusOnError(): void {
    effect(() => {
      ...
      requestAnimationFrame(() => {
        const alertElement = document.querySelector('.customer-detail__error-alert') as HTMLElement | null
        alertElement?.focus()
      })
    })
  }

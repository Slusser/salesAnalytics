import type { OrderCalculationInput, OrderCalculationResult } from '../../pages/orders/new/orders-new.types'
...
export function computeOrderTotals(input: OrderCalculationInput): OrderCalculationResult {
  ...
  const netAfterDistributor = roundCurrency(netAfterProducer * (1 - distributorDiscount / 100))
  const vatAmount = roundCurrency(netAfterDistributor * (vatRate / 100))
  const grossPln = roundCurrency(netAfterDistributor + vatAmount)

  const eurRate = input.currency === 'EUR' ? normalizeNumber(input.eurRate ?? 0) : 0
  const grossEur = input.currency === 'EUR' && eurRate > 0 ? roundCurrency(grossPln / eurRate) : undefined

  const differencePln = roundCurrency(grossPln - net)
  const differenceEur = grossEur != null ? roundCurrency(grossEur - (net / (eurRate || 1))) : undefined

  const withinTolerance = Math.abs(differencePln) <= TOLERANCE && (differenceEur == null || Math.abs(differenceEur) <= TOLERANCE)
